  ...
  <div class="row">
        ...
        @for($col = 0;$col<=$colNum;$col++ )
          <div class="col-md-{{$width}} col-sm-{{$width}} col-xs-12 ">
            @for(; $index<$listLength && $index<$maxlength; $index++ )
              <li title="{{$batchesOfInstitute[$index]->batch}} in {{$batchesOfInstitute[$index]->locality}} - Hyderabad">
                <a href="/batches/show/{{$batchesOfInstitute[$index]->id}}">
                  {{$batchesOfInstitute[$index]->batch.', '.$batchesOfInstitute[$index]->locality}}
                </a>
              </li>
            @endfor
            <?php
      ...
      <div class="col-md-12 col-sm-12 col-xs-12 related_item">
        ...
        @for($col = 0;$col<=$colNum;$col++ )
          <div class="col-md-{{$width}} col-sm-{{$width}} col-xs-12">
            @for(; $index<$listLength && $index<$maxlength; $index++ )
              <li title="{{$subcategoriesInLocality[$index]->subcategory}}">
